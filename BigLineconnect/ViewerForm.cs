...
namespace BigLineconnect
{
    public class ViewerForm : Form
    {
        ...
        {
            this.Text = LanguageManager.Get("title_viewer", _targetId) + " - v3.65.6 (Commercial PRO License & 10-Minute Free Session Limits Engine)";
        ...
        private void ViewerForm_KeyPress(object? sender, KeyPressEventArgs e)
        {
            if (e.KeyChar >= 32)
            {
                byte[] binPkt = BinaryInputProtocol.EncodeChar(e.KeyChar);
                SendBinaryInput(binPkt);
                e.Handled = true;
            }
        }
