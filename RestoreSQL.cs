...
class Program
{
  ...
  SELECT TOP 1 @VarGuid1 = VBag_Guid FROM dbo.VARYANT_BAGLANTI_TANIMLARI 
  WHERE VBag_VaryantKod=@StokKod AND (VBag_KirilimKod=@Renk OR VBag_KirilimKod=@Beden) AND VBag_Tip=0
  IF @VarGuid1 IS NULL SET @VarGuid1 = '00000000-0000-0000-0000-000000000000'
  
  INSERT";
  
        sql = sql.Replace(varGuidLogic, "INSERT");
        ...
        sql = sql.Replace(newInsert, oldInsert);
        
        string newValues = "@BedenNo,@Miktar,0,0,0,@VarGuid1)";
        string oldValues = "@BedenNo,@Miktar,0,0,0)";
        
        sql = sql.Replace(newValues, oldValues);
