...
BEGIN
    ...
    -- KURAL: Sadece Döviz Birim Fiyatı (sth_netagirlik) > 0 olan satırlarda yeniden hesaplama yapılır.
    -- Eğer sth_netagirlik = 0 veya NULL ise (örneğin Masraf satırları), o satırın tutarına HİÇ DOKUNULMAZ ve olduğu gibi korunur.
    UPDATE S
    SET 
    ...
    -- 3. CARI_HESAP_HAREKETLERI Güncellemesi (TÜM Satırların Toplamını Alır: sth_netagirlik > 0 + sth_netagirlik = 0 olan Masraflar)
    UPDATE C
    SET 
...
"@
    ...
    $cmd.ExecuteNonQuery()
    Write-Host "[+] SUCCESS! Trigger updated with core business rule: sth_netagirlik = 0 rows are PRESERVED AS-IS!"
