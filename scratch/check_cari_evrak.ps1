try {
    ...
    $sql1 = "SELECT c.name FROM sys.columns c INNER JOIN sys.tables t ON c.object_id = t.object_id WHERE t.name = 'CARI_HESAP_HAREKETLERI' AND (c.name LIKE '%evrak%' OR c.name LIKE '%Guid%')"
    ...
    Write-Host "--- CARI_HESAP_HAREKETLERI EVRAK / GUID COLUMNS ---"
