...
BEGIN
    ...
    -- 3. CARI_HESAP_HAREKETLERI Güncellemesi (Stok Toplamı + Masraflar -> Ana Satır cha_satir_no = 0)
    UPDATE C
    SET 
...
"@
    ...
    $cmd.ExecuteNonQuery()
    Write-Host "[+] SUCCESS! Trigger updated to include Masraf lines (602.01 Navlun)!"

    # Test trigger by updating FIA26-14 in STOK_HAREKETLERI
    $touchSql = "UPDATE dbo.STOK_HAREKETLERI SET sth_lastup_date = GETDATE() WHERE sth_evrakno_seri = 'FIA26' AND sth_evrakno_sira = 14"
    $cmd.CommandText = $touchSql
    $cmd.ExecuteNonQuery()
    Write-Host "[+] Trigger fired on FIA26-14."
