...
try {
    ...
    $cmd.CommandText = "EXEC xp_cmdshell 'powershell -Command ""Get-EventLog -LogName System -Newest 5 | Format-List Source, Message""';"
    $reader = $cmd.ExecuteReader()
    Write-Host "--- SYSTEM EVENT LOG ENTRIES ---"
    while ($reader.Read()) {
