        ...
        private static async Task SendStreamLoop(WebSocket ws, CancellationToken token)
        {
            ...
            try
            {
                ...
                while (!token.IsCancellationRequested && _isStreaming && (ws.State == WebSocketState.Open || (P2pDirectEngine.IsP2pConnected && (DateTime.Now - _lastP2pAckTime).TotalSeconds < 3.0)))
                {
                    ...
                    // [2026-09-07 düzeltme - "HAYALET YAYIN" (ZOMBIE STREAM) BEKÇİSİ] Viewer normal kapanışta
                    // (pencere X / Alt+F4) STOP_STREAM mesajı gönderiyor ve host bunu görüp _isStreaming=false
                    // yapıyor. AMA viewer çökerse, görev yöneticisinden zorla kapatılırsa ya da ağı aniden
                    // koparsa (kablo/WiFi kaybı), STOP_STREAM HİÇ ULAŞMIYOR — ve host'un Relay sunucusuna olan
                    // WebSocket'i bir süre "Open" görünmeye devam edebiliyor (relay, viewer'ın koptuğunu her
                    // zaman anında host'a bildirmiyor). Bu durumda host, KARŞI TARAFTA KİMSE İZLEMEDİĞİ HALDE
                    // süresiz ekran yayınlamaya devam ediyordu — kullanıcının bildirdiği "bağlantıyı kendim
                    // kopardım, karşı bilgisayarda sadece uygulama açıktı, buna rağmen 5 dk'da 80 MB tüketti"
                    // gözlemiyle tam örtüşen bir senaryo. Viewer canlıyken her 2 saniyede bir PING gönderiyor
                    // (bkz. ViewerForm heartbeat) ve bu, _lastViewerActivityTime'ı tazeliyor; 8 saniye (4 kayıp
                    // PING payı) boyunca PING dahil HİÇBİR sinyal gelmezse viewer'ın gerçekten gittiğini
                    // varsayıp yayını kendiliğinden ve kesin olarak durduruyoruz.
                    if ((DateTime.Now - _lastViewerActivityTime).TotalSeconds > 8)
                    {
                        Log("[Hayalet Yayın Bekçisi] Viewer'dan 8 saniyedir hiçbir sinyal (PING dahil) alınamadı - bağlantı sessizce kopmuş sayılıyor, yayın durduruluyor.");
                        SetStreamActive(false);
                        break;
                    }
                    ...
                    // Over Cloud Relay (!P2pDirectEngine.IsP2pConnected):
                    // NEVER send frame N+1 while frame N is in-flight!
                    // Wait for the Viewer's ACK before sending the next frame.
                    // This strictly guarantees: Host Upload Volume == Viewer Download Volume (0 bytes dropped by relay, 0 buffer bloat).
                    // TIER-AWARE STRICT FLOW CONTROL (ALPEMIX MODEL via AdaptiveRateController):
                    // On Slow3G/Cellular: Max 1 in-flight frame. Strictly wait for Viewer ACK before sending next!
                    // On VDSL: Max 2 in-flight frames.
                    // On Fiber: Max 3 in-flight frames.
                    if (!AdaptiveRateController.CanSendNextFrame(_currentFrameSeq, out _))
                    {
                        // Wait for viewer ACK (up to 40ms per loop iteration)
                        _frameAckEvent.WaitOne(40);
                        if (!AdaptiveRateController.CanSendNextFrame(_currentFrameSeq, out _))
                        {
                            double elapsedSinceSend = (DateTime.Now - _lastFrameSendTime).TotalMilliseconds;
                            if (elapsedSinceSend < 600)
                            {
                            ...
                            {
                                // 600ms safety timeout: network lost an ACK or experienced jitter.
                    ...
                    if (frameToSend != null && frameToSend.Length > 0)
                    {
                        ...
                        if (!isDuplicate || isInitialBurst)
                        {
                            ...
                            int maxAllowedBytesSec = (AdaptiveRateController.CurrentTier == NetworkTier.Slow3G) ? (60 * 1024) : (70 * 1024);
        ...
        private static void ProcessRemoteInput(string json)
        {
            try
            {
                ...
                if (type == "p2p_candidate")
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        if (string.IsNullOrEmpty(P2pDirectEngine.PublicIp) || P2pDirectEngine.PublicPort == 0)
                        {
                        ...
                        {
                            try { await SafeSendAsync(targetCandidateWs, new ArraySegment<byte>(candBytes), WebSocketMessageType.Text, true, CancellationToken.None); } catch { }
                        }
